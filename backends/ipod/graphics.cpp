...
const OSystem::GraphicsMode *OSystem_IPOD::getSupportedGraphicsModes() const {
	return s_supportedGraphicsModes;
}

int OSystem_IPOD::getDefaultGraphicsMode() const {
	return GFX_NORMAL;
}

void OSystem_IPOD::beginGFXTransaction(void) {
...
void OSystem_IPOD::endGFXTransaction(void) {
...
int OSystem_IPOD::getGraphicsMode() const {
	assert (_transactionMode == kTransactionNone);
	return _mode;
}

void OSystem_IPOD::initSize(uint w, uint h) {
...
void OSystem_IPOD::unloadGFXMode() {
	if (_screen) {
...
void OSystem_IPOD::hotswapGFXMode() {
	if (!_screen)
		return;
...
void OSystem_IPOD::internUpdateScreen_Real() {
...
bool OSystem_IPOD::saveScreenshot(const char *filename) {
	assert(_hwscreen != NULL);
...
void OSystem_IPOD::setFullscreenMode(bool enable) {
	Common::StackLock lock(_graphicsMutex);
...
void OSystem_IPOD::setAspectRatioCorrection(bool enable) {
...
void OSystem_IPOD::copyRectToScreen(const byte *src, int pitch, int x, int y, int w, int h) {
	assert (_transactionMode == kTransactionNone);
	assert(src);
...
bool OSystem_IPOD::grabRawScreen(Graphics::Surface *surf) {
	assert(_screen);
	assert(surf);
...
void OSystem_IPOD::addDirtyRect(int x, int y, int w, int h, bool realCoordinates) {
	if (_forceFull)
		return;
...
void OSystem_IPOD::makeChecksums(const byte *buf) {
	assert(buf);
...
void OSystem_IPOD::addDirtyRgnAuto(const byte *buf) {
	assert(buf);
	assert(((long)buf & 3) == 0);
...
int16 OSystem_IPOD::getHeight() {
	return _screenHeight;
}

int16 OSystem_IPOD::getWidth() {
	return _screenWidth;
}

void OSystem_IPOD::setPalette(const byte *colors, uint start, uint num) {
	assert(colors);
	const byte *b = colors;
	uint i;
...
void OSystem_IPOD::grabPalette(byte *colors, uint start, uint num) {
	assert(colors);
...
void OSystem_IPOD::setCursorPalette(const byte *colors, uint start, uint num) {
	assert(colors);
	const byte *b = colors;
	uint i;
...
void OSystem_IPOD::setShakePos(int shake_pos) {
	assert (_transactionMode == kTransactionNone);
...
void OSystem_IPOD::clearOverlay() {
	//assert (_transactionMode == kTransactionNone);
...
void OSystem_IPOD::grabOverlay(OverlayColor *buf, int pitch) {
	assert (_transactionMode == kTransactionNone);
...
void OSystem_IPOD::copyRectToOverlay(const OverlayColor *buf, int pitch, int x, int y, int w, int h) {
	assert (_transactionMode == kTransactionNone);
...
OverlayColor OSystem_IPOD::RGBToColor(uint8 r, uint8 g, uint8 b) {
	return SDL_MapRGB(_overlayscreen->format, r, g, b);
}

void OSystem_IPOD::colorToRGB(OverlayColor color, uint8 &r, uint8 &g, uint8 &b) {
...
bool OSystem_IPOD::showMouse(bool visible) {
...
void OSystem_IPOD::setMousePos(int x, int y) {
...
void OSystem_IPOD::warpMouse(int x, int y) {
	int y1 = y;
...
void OSystem_IPOD::setMouseCursor(const byte *buf, uint w, uint h, int hotspot_x, int hotspot_y, byte keycolor, int cursorTargetScale) {
	if (w == 0 || h == 0)
		return;
...
void OSystem_IPOD::toggleMouseGrab() {
...
#ifdef USE_OSD
void OSystem_IPOD::displayMessageOnOSD(const char *msg) {
	assert (_transactionMode == kTransactionNone);
	assert(msg);
...
void OSystem_IPOD::handleScalerHotkeys(const SDL_KeyboardEvent &key) {
